...
import kr.so.mvc.domain.BoardType;
import kr.so.mvc.domain.MenuType;
import kr.so.mvc.parameter.BoardParameter;
import kr.so.mvc.parameter.BoardSearchParameter;
import kr.so.mvc.service.OriginBoardService;

@Controller
// @RequestMapping("/board")
@Api(tags = "게시판 Api")
public class OriginBoardController {
	Logger logger = LoggerFactory.getLogger(getClass());

	@Autowired
	private OriginBoardService originBoardService;
	 ...
	 * 
	 * List<Board> boardList = originBoardService.getList(pageRequestParameter);
	 * model.addAttribute("boardList", boardList); return new
	 * BaseResponse<List<Board>>(originBoardService.getList(pageRequestParameter)); }
	 * 
	 */
...
/*
	...
	public String detail(@PathVariable MenuType menuType, @PathVariable int boardSeq, Model model) {
		Board board = originBoardService.get(boardSeq);
		if (board == null) {
	...
	public String edit(@PathVariable MenuType menuType, @PathVariable(required = true) int boardSeq, BoardParameter parameter,  Model model) {
		...
		Board board = originBoardService.get(parameter.getBoardSeq());
		if(board == null) {
			throw new BaseException(BaseResponseCode.DATA_IS_NULL, new String[] { "게시물" });
	...
	public BaseResponse<Integer> save(@PathVariable MenuType menuType, BoardParameter parameter) {
		...
		parameter.setBoardType(menuType.boardType());
		originBoardService.save(parameter);
...
// 대용량 등록 처리
	...
	public BaseResponse<Boolean> saveList1() {
		...
		originBoardService.saveList1(list);
	...
	public BaseResponse<Boolean> saveList2() {
		...
		long start = System.currentTimeMillis();
		originBoardService.saveList2(list);
		long end = System.currentTimeMillis();
		logger.info("실행시간 : {}", (end - start) / 1000.0);
		return new BaseResponse<Boolean>(true);
	...
	public BaseResponse<Boolean> delete(@PathVariable int boardSeq) {
		...
		originBoardService.delete(boardSeq);
		return new BaseResponse<Boolean>(true);
	}
