        ...
        methods: {
            ...
            if(resultType === "daily"){
                ...
                success: function (res) {
					...
					tdCnslt : () => {
						$.sendAjax({
                url: "/main/selectTodayCnslt.api",
                data:{},
                contentType: 'application/json',
                success: function (res) {
                    vm.cnsltList = res.data
                    vm.cnsltList.forEach(function(item) {
											
											item.useAt = ''
											if(item.cnsltBeginTime){
												if(item.cnsltBeginTime > exTime && item.onAirAt === "N"){
													item.useAt = '진행 전'
												}else if(item.cnsltBeginTime < exTime && item.onAirAt === "N"){
													item.useAt = '진행 완료'
												}else{
													item.useAt = '진행 중'
												}
												item.cnsltBeginTime = util.cnTime(item.cnsltBeginTime)
											}
											if(item.cnsltEndTime){
												item.cnsltEndTime = util.cnTime(item.cnsltEndTime)
											}
										})
                },
            });
					...
					movCnsltList : () => {
						location.href= window.origin + "/sch/admin/cnsltExprn/consultFx.html"
					},
					movCnsltDetail : (cnsltSeq, cnsltSn) => {
						location.href= window.origin + "/sch/admin/cnsltExprn/consultOperateDetail.html?cnsltSeq=" + cnsltSeq + "&cnsltSn=" + cnsltSn
					},
					tdExprn : () => {
						$.sendAjax({
          ...
          movExprnList : () => {
						location.href= window.origin + "/sch/admin/cnsltExprn/experienceFx.html"
					},
					movExprnDetail : (exprnSeq, exprnSn) => {
						location.href= window.origin + "/sch/admin/cnsltExprn/experienceOperateDetail.html?exprnSeq=" + exprnSeq + "&exprnSn=" + exprnSn
					},
          mainNotice : () => {
						$.sendAjax({
