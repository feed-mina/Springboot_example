...
const profsrUserNum = $("input[name='profsrUserNum']");
const utils = {
    ...
    const app = Vue.createApp({
        ...
        methods: { 
            ...
            fnModalSave: async () => {
                ...
                let cnsltSeq = $("input:radio[name='cnsltSeqNum']:checked").attr('id');
                const paramMap = { 'cnsltSeq': cnsltSeq };

                // 복사하면 시간이랑 교수명을 새로 담기 위해 초기화 필요   vm.cnsltList = []


                const res = await event.sendAjaxRequest("/consult/selectCnslt.api", paramMap);
                await event.setCopyCnslt(res.data);
            ...
            fnModalCancel: function () {
                $("input:radio[name='cnsltSeqNum']").prop('checked', false);
            },
            ...
            fnSave: async (data) => {
                ...
                const payload = {
                    ...
                    startPeriodDt: vm.cnsltData.startPeriodDt|| fixPerdiodDt,
                    endPeriodDt: vm.cnsltData.endPeriodDt || fixEperiodDt
                };
                try {
                    const response = await event.sendAjaxRequest("/consult/insertCnslt.api", payload);

                    $.alert("교수상담 개설 등록이 완료되었습니다. 교수상담 개설 목록으로 이동합니다.", () => {
...
let event = {
    ...
    requireValidation: () => {
        ...
        const validations = [
            ...
            { condition: () => vm.cnsltData.cnsltBeginDe <= vm.cnsltData.rcritEndDt, message: "모집기간 종료일자를 상담시작일자보다 전 날짜로 잡으세요." },
            { condition: () => vm.cnsltData.cnsltEndDe <= vm.cnsltData.cnsltBeginDe, message: "상담종료일자를 상담시작일자 이후로 정해주세요." },
            //{ condition: () => vm.cnsltData.endPeriodDt <= vm.cnsltData.startPeriodDt, message: "모집기간 종료일자를 모집기간 시작일자 이후로 정해주세요." },
            ...
            { condition: () => vm.cnsltData.cnsltBeginDe === undefined, message: "상담 기간 시작일자를 입력해주세요." },
            { condition: () => vm.cnsltData.cnsltEndDe === undefined, message: "상담 기간 종료일자를 입력해주세요." },
            ...
            { condition: () => vm.cnsltData.cnsltBeginDe === "", message: "상담 기간 시작일자를 입력해주세요." },
            { condition: () => vm.cnsltData.cnsltEndDe === "", message: "상담 기간 종료일자를 입력해주세요." }
        ];
    ...
    detailModal: async () => {
        ...
        const response = await event.sendAjaxRequest("/consult/selectConsultList.api", vm.searchData);

        $('.CopyConsult').show();

        vm.cnsltModalList = response.data.list; // 개설 리스트 전체

        for (let i = 0; i < vm.cnsltModalList.length; i++) {
            let modelCnsltArrayStr = vm.cnsltModalList[i].cnsltWeekArray;
            modelCnsltArrayStr = modelCnsltArrayStr.replace("1", "월").replace("2", "화").replace("3", "수").replace("4", "목").replace("5", "금").replace("6", "토").replaceAll(",", "/");
            vm.cnsltModalList[i].cnsltWeekArray = modelCnsltArrayStr;
        }

        fnPaging(response.data.totalCount, dataPerPage, pageCount, response.data.pageNo, (selectPage) => {
            vm.searchData.pageNo = selectPage;
            event.detailModal();
        });
    },
    setCopyCnslt: (resData) => {
        console.log(resData)
       

        // 복사버튼을 누르면 교수명이 인풋 placeholder에 뜬다. 
        //  //console.log(resData)
        $("#professor").children().remove();
        $("#getConsultSnTime2").children().remove();
        $('#modal').modal("hide"); // 닫기
        ...
        for (t = 0; t < cnsltTimeContainerSet; t++) {
            vm.cnsltList.cnsltTimeContainerSet.push(cnsltTimeContainerSet)
            let startTime = vm.cnsltList.cnsltTimeContainer[0][t].split("-")[1].split(' ').join('').replace(/:/g, "")
            let endTime = vm.cnsltList
            let consultTimeSetSession = [startTime, endTime]
        }

        // vm.cnsltList.cnsltTimeContainerSet = [...new Set(vm.cnsltList.cnsltTimeContainer)];
    },
    ...
    getConsultTimeList: (resData) => {
        ...
        //console.log(data)
        let cnsltSeq = vm.cnsltData.cnsltSeq;
        let groupedData = {};
            ...
            if (groupedDataKey[idx] === cnsltSeq) {
                ...
                groupedDataValue[idx].forEach(item => {
                    result.cnsltSn.push(result.cnsltBeginTimeArr.length + 1),
                        ...
                        result.cnsltDt.push(item.cnsltDate),
                    ...
                    result.cnsltEndTimeMinutes.push(item.cnsltEndTimeMinutes),
                        ...
                        result.cnsltEndTimeArr.push(item.cnsltBeginTimeHour + item.cnsltEndTimeMinutes)
                });
    ...
    DeleteCnsltTimeHour: (target) => {
        ...
        if (target.tagName === 'IMG') {
            ...
            //console.log(Array.from(new Set(vm.cnsltList.reCnsltDt)))
            //console.log(vm.cnsltList)
