	...
	const app = Vue.createApp({
		...
		methods: {
    		...
    		event.editorToTextarea();
				...
				if(util.sjChk(vm.faqData.bbsSj)){
				...
				let valiChk = event.removeHtml(vm.faqData.bbsCn);
				if(util.cnChk(vm.faqData.bbsCn)){
					$.alert("내용은 2000자 이내로 작성해주세요.");
					return false;
				}
				
				let formData = new FormData();
    		formData.append('paramMap', new Blob([JSON.stringify(vm.faqData)], {type: 'application/json'}));
