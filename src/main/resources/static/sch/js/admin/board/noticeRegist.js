...
let event = {
	init: () =>{
		...
		nhn.husky.EZCreator.createInIFrame({
	      ...
	      fOnAppLoad : function(){
			  	oEditors.getById['bbsCn'].setDefaultFont( "돋움", 12);  // 기본 글꼴 설정 추가
					//예제 코드
					//oEditors.getById["bbsCn"].exec("PASTE_HTML", ["내용을 입력해 주세요."]);
				},
  ...
  removeHtml: function (text) {
    ...
    event.editorToTextarea();
		...
		if(util.sjChk(vm.noticeData.bbsSj)){
			$.alert("제목은 100자 이내로 작성해주세요.");
			return false;
		}
		
		if(vm.noticeData.useAt === ''){
			$.alert("노출상태를 선택하세요.");
			return false;
		}
		
		let valiChk = event.removeHtml(vm.noticeData.bbsCn);
		if(util.cnChk(valiChk)){
			$.alert("내용은 2000자 이내로 작성해주세요.");
			return false;
		}
		
		if(vm.noticeData.bbsSe === 'B'){
			if(vm.filesList.length){
				for (let i = 0; i < vm.filesList.length; i++) {
					if (vm.filesList[i].type.split('/')[0] !== "video") {
		...
		}else if(vm.noticeData.bbsSe === 'I'){
			if(vm.filesList.length){
				for (let i = 0; i < vm.filesList.length; i++) {
					if (vm.filesList[i].type.split('/')[0] !== "image") {
		...
		if(vm.noticeData.bannerType === 'video'){
			vm.noticeData.bannerNm = 'adVideo.mp4'
		}else if(vm.noticeData.bannerType === 'image'){
			vm.noticeData.bannerNm = 'adImage.png'
		}
    ...
    if(vm.noticeData.useAt === 'Y' && (vm.noticeData.bbsSe === 'B' || vm.noticeData.bbsSe === 'I')){
			$.sendAjax({
      ...
      cache: false,
			success : (res) => {
				$.alert("공지사항 등록이 완료되었습니다. <br>공지사항 목록으로 이동합니다.", () => {
	...
	fnCancel: () => {
		$.confirm("지금까지 입력한 내용이 모두 사라집니다. 정말 취소하시겠습니까?", () => {
			...
			vm.noticeData.useAt = "";
			vm.filesList = [];
			oEditors.getById["bbsCn"].exec("SET_IR", [""]); //내용초기화
    ...
    fileValidation: function (obj) {
        ...
        if (obj.name.length > 100) {
			...
			vm.noticeData.bannerType = obj.type.split('/')[0]
            return true;
        }
    }
